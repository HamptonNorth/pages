...
document.addEventListener('DOMContentLoaded', function () {
  const searchCountriesButton = document.getElementById('searchCountriesBtn')
  if (searchCountriesButton) {
    searchCountriesButton.addEventListener('click', function () {
      searchCountries()
    })
  } else {
    console.error('Button with ID "searchCountriesBtn" not found.')
  }
})
    ...
    if (countries.length === 0) {
      ...
      resultDiv.innerHTML = `
              <div class=""> Countries containing:  <span class="pl-2 font-semibold text-sm">${term}</span>
                ...
                <div class="overflow-x-auto w-full border border-slate-200 rounded-lg shadow-sm mt-4">
