...
/**
 * Initialize the test_countries table with sample data
 * @param {Database} db - The database instance
 * @param {string} tableName - Name of the table to create
 */
export function initialiseTestCountries(db, tableName) {
  const testCountriesExists = tableExists(db, tableName)

  if (testCountriesExists) {
    console.log(`${tableName} table already exists`)
    return
  }

  // Create example/test table
  db.run(
  ...
  // Insert sample data
  db.run(
    `INSERT INTO test_countries (rank, iso_code, iso_name, official_state_name, currency, currency_code, tld, population, gdp_billions_usd) VALUES
...
`,
  ...
  console.log(`${tableName} table populated with sample data`)
}

export function initialiseTestUsers(db, tableName = 'test_users') {
  const testUsersExists = tableExists(db, tableName)
  ...
  // Insert sample data
  // Use positional parameters for Bun SQLite
  const sql = `INSERT INTO test_users (id, user_name, first_name, surname, status_setting, date_added, date_last_amended) VALUES
      (?, ?, ?, ?, ?, ?, ?);`

  const stmt = db.prepare(sql)

  const insertUsersInTransaction = db.transaction((users) => {
    for (const user of users) {
      stmt.run(
        ...
        user.status_setting,
        user.date_added,
        user.date_last_amended,
      )
    }
    // Return the number of users inserted for verification
    return users.length
  })

  try {
    const insertedCount = insertUsersInTransaction(usersToInsert)
    console.log(`test_users table populated with ${insertedCount} users.`)
  } catch (error) {
    console.error(' Insert transaction to user_table failed:', error)
  }
}
